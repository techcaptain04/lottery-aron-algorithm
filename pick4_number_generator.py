"""
Read Pick4-style CSV (newest draw first) and build 20-number groups for each
match on the midday or evening column: the match row plus the nine draws
listed above it in the file (10 rows x Midday + Evening = 20 values).

For each window value, prints one line: the digits sorted ascending (e.g.
6204->246, 7888->7888). Then prints a duplicate report: values that appear
more than once in that full list, with counts and which raw draw numbers
produced each value.
"""
...
def load_draws(path: Path) -> list[tuple[str, int | None, int | None]]:
    ...
    with path.open(newline="", encoding="utf-8-sig") as f:
        ...
        for r in reader:
            ...
            mid = parse_optional_int(r.get("Midday Win 4 #"))
            eve = parse_optional_int(r.get("Evening Win 4 #"))
...
def sorted_digits_asc_str(n: int) -> str:
    """Sort decimal digits ascending, then normalize as int string (6204 -> 246, 7888 -> 7888)."""
...
def export_duplicates_csv(
    ...
    out_path = Path(f"pick4_{date_part}_{draw}.csv")
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Extract 20-number windows for each Pick4 midday or evening winning-number match.",
    )
    p.add_argument(
        "csv",
        type=Path,
        help="Path to the Pick4 CSV file",
    )
    p.add_argument(
        "-d",
        "--draw",
