...
nine draws listed above it in the file (10 rows x Midday + Evening = 20 values).

For each window value (after dropping every copy of the search target `-n`),
prints one line: the digits sorted ascending (e.g. 6204->246, 7888->7888).
Then prints a duplicate report: values that appear more than once in that
list, with counts and which raw draw numbers produced each value.
"""
